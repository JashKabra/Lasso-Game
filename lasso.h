...
#define COIN_ANGLE_DEG_LEVEL1 90
#define COIN_ANGLE_DEG_LEVEL2 60
...
class Lasso : public MovingObject {
 ...
 Lasso(double speed, double angle_deg, double argax, double argay, bool argpaused, bool rtheta) : MovingObject(speed, angle_deg, argax, argay, argpaused, rtheta) {
  ...
  void check_for_both_coins(Coin *coin1, Coin*coin2);
  void check_for_negativecoin(Coin *bomb);
  int getNumCoins() { return num_coins; }
  int getLevel(){return (level/2)+1;}
  int getSpeed(){return release_speed;}
  bool check_for_coin_speed(Coin *coin);
