...
void MovingObject::reset_cord(double argx, double argy){
  for(size_t i=0; i<parts.size(); i++){
  parts[i]->moveTo(argx, argy);
  }
} // Only resets the x and y co ordinates

void MovingObject::reset_speed(double speed, double angle_deg, double argax, double argay, bool argpaused, bool rtheta) {
  double angle_rad = angle_deg*PI/180.0;
  double argvx = speed*cos(angle_rad);
  double argvy = -speed*sin(angle_rad);
  vx = argvx; vy = argvy; ax = argax; ay = argay; paused = argpaused;
